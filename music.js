...
app.get('/', (req, res) => res.redirect('/music.html')); // Guests can now browse
...
app.post('/api/register', (req, res) => {
    ...
    // EXACT STRING MATCH FOR FRONTEND UI
    if (users.find(u => u.username.toLowerCase() === username.toLowerCase())) return res.status(400).send('USER HAS BEEN REGISTERED');
    if (users.find(u => u.contact.toLowerCase() === contact.toLowerCase())) return res.status(400).send('USER HAS BEEN REGISTERED');
