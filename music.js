...
function uploadStreamToCloudinary(buffer, resourceType, folder) {
    return new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream( { folder: folder, resource_type: resourceType }, (error, result) => { if (error) reject(error); else resolve(result); } );
...
// --- VIP ROUTE ---
app.post('/api/users/:username/vip', async (req, res) => {
    try {
        if(!db) return res.status(500).send('Database not connected');
...
app.post('/api/users/:username/profile-pic', async (req, res) => {
    try { const url = await uploadToCloudinaryBase64(req.body.imageBase64, 'dj_profiles'); await db.collection('users').doc(req.params.username.toLowerCase()).update({ profilePic: url }); res.json({ profilePic: url }); } catch (e) { res.status(500).send(e.message); }
});
...
app.put('/api/genres/reorder', async (req, res) => { try { const batch = db.batch(); req.body.orderedIds.forEach((id, index) => { batch.update(db.collection('genres').doc(id), { sequence: index + 1 }); }); await batch.commit(); res.send('Reordered'); } catch(e) { res.status(500).send(e.message); }});
...
async function saveSongData(fileBuffer, originalName, reqBody) {
    ...
    const newSong = { filename: reqBody.title || originalName, filepath: url, coverUrl: coverUrl, genreId: reqBody.genreId || 'none', size: fileBuffer.length, uploadTime: new Date().toISOString(), sequence: snapshot.size + 1, price: parseInt(reqBody.price) || 10 };
...
app.post('/api/transload', async (req, res) => { try { const response = await fetch(req.body.url); if (!response.ok) throw new Error(`HTTP Error from source URL`); const buffer = Buffer.from(await response.arrayBuffer()); res.json(await saveSongData(buffer, 'TransloadedTrack.m4a', req.body)); } catch (e) { res.status(400).send(e.message); }});
app.put('/api/songs/:id/settings', async (req, res) => { let updates = {}; if (req.body.newName) updates.filename = req.body.newName; if (req.body.newPrice !== undefined) updates.price = parseInt(req.body.newPrice) || 0; await db.collection('songs').doc(req.params.id).update(updates); res.send('Updated');});
...
app.get('/api/settings', async (req, res) => { if(!db) return res.json({ headerTitle: 'MusicScraper', heroTitle: '专属DJ节奏空间', bannerUrl: '' }); const doc = await db.collection('settings').doc('global').get(); res.json(doc.exists ? doc.data() : { headerTitle: 'MusicScraper', heroTitle: '专属DJ节奏空间', bannerUrl: '' });});
